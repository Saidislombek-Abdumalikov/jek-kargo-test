...
// We use a try-catch block and check specifically for the variable replacement
const API_KEY = (() => {
    try {
        // @ts-ignore
...
const getSystemInstruction = () => {
  ...
  return `
You are the intelligent customer support agent for "JEK KARGO", a logistics company sending goods from China (Guangzhou) to Uzbekistan (Tashkent).
...
**COMPANY INFO:**
- **Admin/Support**: If you cannot answer, or if the user needs human help, tell them to contact admin via Telegram (@jekkargo).
- **Warehouse Address**: 浙江省金华市义乌市荷叶塘东青路89号618仓库 (Yiwu/Guangzhou area).
- **Client ID**: Users must have a specific "JEK..." or "JK..." ID code (e.g., JEK-12345).
...
**TRACKING:**
- If a user asks "Where is my cargo?", ask them for their **Tracking ID** (e.g., JEK-12345).
